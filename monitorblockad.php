...
$time = get_option('adblock_script_time');
$close = get_option('adblock_script_close');

if(empty($time) || $time%1 !=0){
  $time = 0;
}
...
<span id='adblock_screen' style='display:none'><font id='rba2' style='z-index:2000'>";
if (!empty($close)) {
  echo "<span style='position: absolute;margin-top: -120px;margin-left: 740px; cursor:pointer' onclick='close_screen();'>X</span>";
}

echo "<b>" . $m . "</b>&nbsp&nbsp&nbsp&nbsp<button type='button'  class='btnadb' onClick='window.location.reload()'>Refresh&nbsp<b>↻</b></button><br><br>";
...
e.blockAdBlock&&(e.blockAdBlock=new d({checkOnLoad:!0,resetOnEnd:!0}))})(window);var b=window.blockAdBlock;b.onDetected(function(){setTimeout(function show(){document.getElementById('adblock_screen').style.display='block';}, " . $time . ")});b.onNotDetected(function(){});
...
add_option('adblock_script_time');
add_option('adblock_image_url');
add_option('adblock_script_close');
...
function easy_ma_register_my_setting(){
  ...
  register_setting('adblock_options', 'adblock_script_time');
  register_setting('adblock_options', 'adblock_image_url');
  register_setting('adblock_options', 'adblock_script_close');
}
